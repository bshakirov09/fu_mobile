...
class GetJournalDetailRequest {
  Future<Either<String, ProgressJournalDetailModel>> getData({
    required int id,
  }) async {
    try {
      final Response response = await getIt.get<Dio>().get(
            "${APIList.getJournalDetailProgress}$id/",
            options: Options(
                headers: {
      ...
      if (response.statusCode == HttpStatus.ok) {
        return right(ProgressJournalDetailModel.fromJson(response.data));
