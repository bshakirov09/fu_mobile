...
class GetWorkoutMainDetailRequest {
  Future<Either<String, WorkoutMainDetailModel>> getData({
    required int id,
  }) async {
    try {
      final Response response = await getIt.get<Dio>().get(
            "${APIList.getWorkoutMainDetail}$id/",
            options: Options(
                headers: {
      ...
      if (response.statusCode == HttpStatus.ok) {
        return right(WorkoutMainDetailModel.fromJson(response.data));
